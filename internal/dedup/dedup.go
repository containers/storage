...
func DedupDirs(dirs []string, options DedupOptions) (DedupResult, error) {
	...
	for _, dir := range dirs {
		...
		if err := pwalkdir.Walk(dir, func(path string, d fs.DirEntry, err error) error {
			if err != nil {
				logrus.Errorf("Failed to read file %q: %v", path, err)
				return nil
			}
			if !d.Type().IsRegular() {
				return nil
			...
			info, err := d.Info()
			if err != nil {
				logrus.Errorf("Failed to get file info for %q: %v", path, err)
				return nil
			}
			...
			if visited, err := dedup.isFirstVisitOf(info); err != nil {
				logrus.Errorf("Failed to check if file was visited: %v", err)
				return nil
			} else if visited {
				return nil
			}
			...
			if err != nil {
				logrus.Errorf("Failed to get checksum for %q: %v", path, err)
				return nil
			}
