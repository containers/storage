...
func TestCreateAndLockAndTryLock(t *testing.T) {
	l, path, err := CreateAndLock(t.TempDir(), "locktest")
	require.NoError(t, err)
...
func TestUnlockAndDeleteTwice(t *testing.T) {
	l, path, err := CreateAndLock(t.TempDir(), "panic-unlockdelete")
	require.NoError(t, err)
	defer os.Remove(path)
	require.NoError(t, l.UnlockAndDelete())
...
func TestLockFileRecreation(t *testing.T) {
	l, path, err := CreateAndLock(t.TempDir(), "recreate-lock")
	require.NoError(t, err)
	require.NoError(t, l.UnlockAndDelete())
...
func TestTryLockPathMultiProcess(t *testing.T) {
	lockfile, path, err := CreateAndLock(t.TempDir(), "test-staging-lockfile")
	require.NoError(t, err)

	expectedErrMsg := fmt.Sprintf("error opening lock file %q: failed to acquire lock on ", path)
	tryLockTimes := 3
	for i := 0; i < tryLockTimes; i++ {
		cmd, stdout, err := subTryLockPath(path)
		require.NoError(t, err)
