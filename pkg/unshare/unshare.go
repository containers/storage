...
func getHostIDMappingsProc(path string) ([]specs.LinuxIDMapping, error) {
...
func getHostIDMappings(pid string) ([]specs.LinuxIDMapping, []specs.LinuxIDMapping, error) {
	...
	uidmap, err := getHostIDMappingsProc(fmt.Sprintf("/proc/%s/uid_map", pid))
	if err != nil {
		return nil, nil, err
	}
	gidmap, err := getHostIDMappingsProc(fmt.Sprintf("/proc/%s/gid_map", pid))
	if err != nil {
		return nil, nil, err
	}
...
func unshareCmd(args []string) error {
