...
func initCache(sizeCache int) (*cacheFile, string, string, *bloomFilter) {
	...
	bloomFilter := newBloomFilter(uint(sizeCache)*factorNM, numberHashes)
	...
	for i := 0; i < sizeCache; i++ {
		...
		digestLen = len(d)
		presentDigest = d
		tag, err := generateTag([]byte(d), 0, 0)
		if err != nil {
			panic(err)
		}
		tagLen = len(tag)
		tags = append(tags, tag)
		bd, err := getBinaryDigest(d)
		if err != nil {
			panic(err)
		}
		bloomFilter.add(bd)
...
func init() {
	...
	presentDigestInCacheBinary, err = getBinaryDigest(presentDigestInCache)
	if err != nil {
		panic(err)
	}
	notPresentDigestInCacheBinary, err = getBinaryDigest(notPresentDigestInCache)
	if err != nil {
		panic(err)
	}
}
