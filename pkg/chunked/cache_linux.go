...
func generateFileLocation(pathPos int, offset, len uint64) ([]byte, error) {
	var buf []byte
...
func bloomFilterFromTags(tags [][]byte, digestLen int) *bloomFilter {
	bloomFilter := newBloomFilter(uint(len(tags)*bloomFilterScale), bloomFilterHashes)
	for _, t := range tags {
...
func writeCache(manifest []byte, format graphdriver.DifferOutputFormat, id string, dest setBigData) (*cacheFile, error) {
	...
	for _, k := range toc {
		if k.Digest != "" {
			digest, err := getBinaryDigest(k.Digest)
			if err != nil {
				return nil, err
			}
			...
			location, err := generateFileLocation(fileNamePos, 0, uint64(k.Size))
			if err != nil {
				return nil, err
			}
			...
			digestHardLink, err := getBinaryDigest(fp)
			if err != nil {
				return nil, err
			}
		...
		if k.ChunkDigest != "" {
			...
			location, err := generateFileLocation(fileNamePos, uint64(k.ChunkOffset), uint64(k.ChunkSize))
			if err != nil {
				return nil, err
			}
			off := uint64(vdata.Len())
			...
			digest, err := getBinaryDigest(k.ChunkDigest)
			if err != nil {
				return nil, err
			}
			d, err := generateTag(digest, off, l)
...
func findTag(digest string, cacheFile *cacheFile) (string, uint64, uint64) {
	binaryDigest, err := getBinaryDigest(digest)
	if err != nil {
		return "", 0, 0
	}
