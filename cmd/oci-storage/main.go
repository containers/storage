...
func main() {
	...
	makeFlags := func(command string, eh mflag.ErrorHandling) *mflag.FlagSet {
		...
		flags.StringVar(&options.GraphMap.Root, []string{"-graph", "g"}, options.GraphMap.Root, "Root of the storage tree")
		flags.StringVar(&options.GraphMap.DriverName, []string{"-storage-driver", "s"}, options.GraphMap.DriverName, "Storage driver to use ($STORAGE_DRIVER)")
		flags.Var(opts.NewListOptsRef(&options.GraphMap.DriverOptions, nil), []string{"-storage-opt"}, "Set storage driver options ($STORAGE_OPTS)")
		...
		for _, name := range command.names {
			if cmd == name {
				...
				if debug {
					...
					logrus.Debugf("GraphRoot: %s", options.GraphMap.Root)
					logrus.Debugf("GraphDriverName: %s", options.GraphMap.DriverName)
					logrus.Debugf("GraphDriverOptions: %s", options.GraphMap.DriverOptions)
				} else {
